...
# Function to kill processes running on port 8010
kill_port_8010() {
    log_with_timestamp "Killing any process using port 8010..."
    pids=$(lsof -t -i:8010)
    if [ -n "$pids" ]; then
        for pid in $pids; do
            kill -9 "$pid" && log_with_timestamp "Successfully killed process $pid on port 8010." || log_with_timestamp "Failed to kill process $pid on port 8010."
        done
    else
        log_with_timestamp "No process found on port 8010."
    fi
}
...
restart_application() {
    ...
    /home/hello/.nvm/versions/node/v18.18.2/bin/pm2 start ecosystemtruoc.config.js || {
